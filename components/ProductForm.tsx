      ...
      <div className={`bg-[#2d0606] border border-white/10 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in ${isFastMode ? 'border-blue-500/30' : 'border-orange-500/20'}`}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (validate()) onSubmit(formData); }} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className={`flex-1 bg-black/20 border rounded-lg py-2.5 px-4 text-white uppercase transition-all outline-none ${isNameLocked ? 'opacity-60 cursor-not-allowed border-blue-500/30' :
              ...
              {matchFound && isFastMode && (
                <button
                  ...
                  className={`px-4 rounded-lg flex items-center justify-center transition-all border-b-4 ${isNameLocked
          ...
          <footer className="pt-4 flex flex-wrap gap-3">
            <button
              type="submit"
              className={`flex-1 font-black uppercase text-[11px] py-4 rounded-xl shadow-xl border-b-4 transition-all active:translate-y-0.5 active:border-b-0 ${isFastMode
